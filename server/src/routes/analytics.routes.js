...
export default async function analyticsRoutes(fastify) {
  fastify.get('/api/analytics/financial', { preHandler: [fastify.authenticate, fastify.requirePermission('reports.view', 'ceo', 'gm', 'finance', 'accountant', 'staff', 'cashier', 'fuel')] }, async (request, reply) => {
    ...
    const [sales, expenses, branches, products, fuelShifts] = await Promise.all([
    ...
    return { filters: { start, end, branch: request.query?.branch, product: request.query?.product, shift: request.query?.shift }, summary: personalRole ? personalSummary : operatingSummary, monthly: personalRole ? monthly.map(({month,revenue})=>({month,revenue})) : monthly, forecast: personalRole ? [] : forecast, fuelShifts,
      byBranch: [...branchMap].map(([name,value])=>({name,value:round(value)})).sort((a,b)=>b.value-a.value), topProducts: [...productMap].map(([name,value])=>({name,value:round(value)})).sort((a,b)=>b.value-a.value).slice(0,10), products: personalRole ? products.map(({_id,name,code,category})=>({_id,name,code,category})) : products };
  });
}
