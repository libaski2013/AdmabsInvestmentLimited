    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden self-start">
          ...
          <div className="px-4 pb-3 flex gap-2"><span className="text-xs font-bold text-gray-500 py-1">Shift:</span>{['day','night'].map(s=><button key={s} onClick={()=>setWorkShift(s)} className={`px-3 py-1 rounded-full text-xs font-bold ${workShift===s?'bg-red-600 text-white':'bg-gray-100 text-gray-500'}`}>{s==='day'?'Day':'Night'}</button>)}</div>
...
const SECTIONS = { dash: DashView, pos: PosView, mkt: MktView, fuel: FuelView, inv: InvView, proc: ProcView, expenses: ExpensesView, fin: FinView, cust: CustView, reconcile: ReconcileView, staff: StaffView, rep: RepView, sett: SettView, website: WebsiteView, payroll: PayrollView, sms: SmsView, tools: AdminToolsView };
