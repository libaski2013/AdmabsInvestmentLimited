...
function InvView({ user }) {
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-xl font-black text-blue-900">Inventory & Services by Outlet</h2><p className="text-sm text-gray-500">Automotive stock, supermarket stock and non-stock services are managed separately at each outlet</p></div><div className="flex flex-wrap gap-2"><div className="flex border rounded-xl bg-white p-1"><button onClick={()=>setView('thumbnail')} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${view==='thumbnail'?'bg-blue-900 text-white':'text-gray-500'}`}>▦ Thumbnails</button><button onClick={()=>setView('list')} className={`px-3 py-1.5 rounded-lg text-xs font-bold ${view==='list'?'bg-blue-900 text-white':'text-gray-500'}`}>☰ List</button></div>{canEdit && <>{division!=='services'&&<button onClick={()=>setScanner(true)} className="px-4 py-2 border border-blue-200 text-blue-900 rounded-xl text-xs font-black">▣ Scan to Stock</button>}<button onClick={() => { setEdit('new'); setForm({ category:division==='supermarket'?'Grocery':division==='services'?'Service':'Tyre', qty:0, reorderLevel:division==='services'?0:5, price:0, cost:0, websiteVisible:true }); }} className="px-4 py-2 bg-red-600 text-white rounded-xl text-xs font-black">{division==='services'?'+ Add Service':'+ Add Product'}</button></>}</div></div>
