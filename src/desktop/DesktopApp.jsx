...
function CustView({ user }) {
  ...
  const [statement,setStatement]=useState(null); const [statementFilters,setStatementFilters]=useState({start:'',end:'',branch:'',outlet:''}); const [payment,setPayment]=useState(null); const [paymentReceipt,setPaymentReceipt]=useState(null); const [site,setSite]=useState({}); const [statementQr,setStatementQr]=useState('');
  ...
  return (
    <div className="space-y-5">
      {statement&&sel&&<div className="sticky top-0 z-20 flex justify-end gap-2 bg-gray-50 py-2"><button onClick={()=>shareStatement('view')} className="border border-blue-200 bg-white text-blue-800 rounded-xl px-4 py-2.5 text-xs font-black">View PDF</button><button onClick={()=>shareStatement('share')} className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl text-xs font-black shadow-sm"><Send size={14}/> Share PDF via WhatsApp / Email</button></div>}
