...
function CustView({ user }) {
  ...
  const shareStatement=async()=>{
    ...
    y=Math.min(y+5,280);pdf.setFont('helvetica','bold');pdf.text(`Total purchases: ${fmt(statement.summary.purchaseTotal)}`,196,y,{align:'right'});pdf.text(`Payments received: ${fmt(statement.summary.paymentsReceived)}`,196,y+6,{align:'right'});pdf.setTextColor(190,24,24);pdf.text(`BALANCE DUE: ${fmt(statement.summary.balance)}`,196,y+12,{align:'right'});
    const safeName=sel.name.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'');const filename=`ADMABS-Statement-${safeName}.pdf`;const blob=pdf.output('blob');const file=new File([blob],filename,{type:'application/pdf'});const message=`ADMABS customer account statement for ${sel.name}. Balance due: ${fmt(statement.summary.balance)}.`;
