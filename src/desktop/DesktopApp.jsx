...
function CustView({ user }) {
  ...
  const shareStatement=async(mode='share')=>{
    ...
    const widths=[33,46,30,21,21,18];for(const row of rows){const cells=row.map((value,i)=>pdf.splitTextToSize(String(value),widths[i]));const rowHeight=Math.max(9,...cells.map(lines=>lines.length*3.5+3));if(y+rowHeight>278){pdf.addPage();y=16}cells.forEach((lines,i)=>pdf.text(lines,xs[i],y));pdf.setDrawColor(225);pdf.line(12,y+rowHeight-2,198,y+rowHeight-2);y+=rowHeight}
