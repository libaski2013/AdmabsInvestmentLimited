...
function CustView({ user }) {
  ...
  const shareStatement=async(mode='share')=>{
    ...
    const headers=['Date / Reference','Branch / Outlet','Transaction','Debit','Credit','Due']; const xs=[14,50,100,133,156,179];pdf.setFillColor(20,55,130);pdf.rect(12,y,186,8,'F');pdf.setTextColor(255);pdf.setFont('helvetica','bold');pdf.setFontSize(7.5);headers.forEach((h,i)=>pdf.text(h,xs[i],y+5));y+=10;pdf.setTextColor(15,23,42);pdf.setFont('helvetica','normal');pdf.setFontSize(7.5);
    ...
    const widths=[33,46,30,21,21,18];for(const row of rows){if(y>278){pdf.addPage();y=16}row.forEach((value,i)=>pdf.text(String(value),xs[i],y,{maxWidth:widths[i]}));pdf.setDrawColor(225);pdf.line(12,y+3,198,y+3);y+=9}
