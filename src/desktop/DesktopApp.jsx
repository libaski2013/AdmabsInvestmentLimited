          ...
          {sel && (
            <div className="lg:col-span-3">
              <div className="bg-blue-900 rounded-xl p-5 text-white">
                <div className="flex items-start justify-between gap-3 mb-4"><div><h3 className="font-black text-lg">{sel.name}</h3><p className="text-sm opacity-70">{sel.type} · {sel.visits} visits</p></div><div className="flex gap-2">{Number(sel.balance)>0&&<button onClick={()=>setPayment({amount:sel.balance,method:'Cash',reference:'',notes:''})} className="text-xs font-black bg-red-600 px-3 py-2 rounded-lg">Receive Payment</button>}{canManageCustomers&&<button onClick={()=>{setModal(sel);setForm(sel);}} className="text-xs font-bold bg-white/10 px-3 py-2 rounded-lg">Edit Customer</button>}</div></div>
