...
function CustView({ user }) {
  ...
  const shareStatement=async(mode='share')=>{
    ...
    pdf.setFontSize(9);pdf.text(sel.name,14,y);pdf.setFont('helvetica','normal');pdf.text(`${sel.phone||''} ${sel.email?`| ${sel.email}`:''}`,14,y+5);pdf.text(`Generated: ${new Date().toLocaleString()}`,196,y,{align:'right'});pdf.text(`Period: ${statementFilters.start||'Beginning'} to ${statementFilters.end||'Today'}`,196,y+5,{align:'right'});y+=14;
    ...
    if(confirm('The PDF has been downloaded. Open WhatsApp to send it? Select the downloaded PDF as the attachment.'))window.open(`https://wa.me/${String(sel.phone||'').replace(/\D/g,'')}?text=${encodeURIComponent(message)}`,'_blank');else window.location.href=`mailto:${encodeURIComponent(sel.email||'')}?subject=${encodeURIComponent('ADMABS Customer Account Statement')}&body=${encodeURIComponent(`${message}\n\nPlease attach the downloaded PDF: ${filename}`)}`;
  };

  return (
    <div className="space-y-5">
      {statementActionsHost&&createPortal(<><button onClick={()=>shareStatement('view')} className="border border-blue-200 bg-white text-blue-800 rounded-lg px-3 py-2 text-xs font-bold">View PDF</button><button onClick={()=>shareStatement('share')} className="bg-green-600 text-white rounded-lg px-3 py-2 text-xs font-bold"><Send size={13} className="inline mr-1"/>WhatsApp / Email</button></>,statementActionsHost)}
