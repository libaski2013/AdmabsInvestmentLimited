...
function GlobalHelpTooltip() {
  ...
  return tip?createPortal(<div className="fixed z-[100] max-w-64 rounded-lg bg-slate-950 px-3 py-2 text-xs font-semibold text-white shadow-2xl pointer-events-none admabs-tooltip" style={{left:tip.x,top:tip.y}}>{tip.text}</div>,document.body):null;
}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden self-start">
            ...
            {pos.cart.map((item, i) => (
              <div key={i} className="bg-gray-50 rounded-xl p-2.5">
                <div className="flex items-start justify-between gap-2 mb-2">
                  ...
                  <button onClick={() => pos.removeItem(item.id)} title="Remove item from cart" className="flex items-center gap-1 text-red-500 hover:text-red-700 text-xs font-bold flex-shrink-0"><X size={11} /> Remove</button>
                </div>
          ...
          <div className="p-3 pt-0 grid grid-cols-3 gap-2">
            <button onClick={()=>window.confirm('Cancel this sale and clear the cart?')&&pos.cancelSale()} disabled={pos.cart.length===0||pos.saving} className="border border-red-200 text-red-600 disabled:opacity-40 font-bold text-xs py-3 rounded-xl">Cancel</button>
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map((p, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    ...
                    <td className="px-4 py-2.5 font-bold"><span className={p.category==='Service'?'text-gray-400':p.qty <= p.reorderLevel ? 'text-red-600' : 'text-gray-800'}>{p.category==='Service'?'N/A':p.qty}</span></td>
...
function CustView({ user }) {
  ...
  useEffect(()=>{if(!statement||!selId){setStatementQr('');return}import('qrcode').then(({default:QRCode})=>QRCode.toDataURL(`ADMABS|CUSTOMER-STATEMENT|${selId}|${Date.now()}|${statement.summary.balance}`,{width:320,margin:1,errorCorrectionLevel:'M'})).then(setStatementQr).catch(()=>{})},[statement,selId]);
  ...
  const shareStatement=async(mode='share')=>{
    ...
    pdf.setFontSize(9);pdf.text(`CUSTOMER NAME: ${sel.name||'Not provided'}`,14,y);pdf.setFont('helvetica','normal');pdf.text(`PHONE NUMBER: ${sel.phone||'Not provided'}`,14,y+5);pdf.text(`CUSTOMER LOCATION: ${customerLocation}`,14,y+10,{maxWidth:112});pdf.setFont('helvetica','bold');pdf.setTextColor(190,24,24);pdf.text(`ACCOUNT BALANCE DUE: ${pdfMoney(statement.summary.balance)}`,14,y+17);pdf.setTextColor(15,23,42);pdf.setFont('helvetica','normal');pdf.text(`Generated: ${new Date().toLocaleString()}`,196,y,{align:'right'});pdf.text(`Period: ${statementFilters.start||'Beginning'} to ${statementFilters.end||'Today'}`,196,y+5,{align:'right'});y+=26;
    const headers=['Date / Reference','Branch / Outlet','Transaction','Customer Debt','Customer Payments','Balance Due']; const xs=[14,50,100,133,156,179];pdf.setFillColor(20,55,130);pdf.rect(12,y,186,9,'F');pdf.setTextColor(255);pdf.setFont('helvetica','bold');pdf.setFontSize(7.5);headers.forEach((h,i)=>pdf.text(h,xs[i],y+6));y+=14;pdf.setTextColor(15,23,42);pdf.setFont('helvetica','normal');pdf.setFontSize(7.5);
    const rows=[...statement.purchases.map(x=>[`${new Date(x.createdAt).toLocaleDateString()} ${x.invoiceNumber}`,`${x.branch?.name||'Head Office'} / ${x.outlet?.name||''}`,'Purchase',pdfMoney(x.total),pdfMoney(x.paidAtSale+x.paidLater),pdfMoney(x.amountDue)]),...statement.payments.map(x=>[`${new Date(x.receivedAt).toLocaleDateString()} ${x.receiptNumber}`,`${x.branch?.name||'Head Office'} / ${x.outlet?.name||''}`,`Payment - ${x.method}`,'-',pdfMoney(x.amount),'-'])];
    const widths=[33,46,30,21,21,18];for(const row of rows){const cells=row.map((value,i)=>pdf.splitTextToSize(String(value),widths[i]));const rowHeight=Math.max(9,...cells.map(lines=>lines.length*3.5+3));if(y+rowHeight>250){pdf.addPage();y=16}cells.forEach((lines,i)=>pdf.text(lines,xs[i],y));pdf.setDrawColor(225);pdf.line(12,y+rowHeight-2,198,y+rowHeight-2);y+=rowHeight}
    y=Math.min(y+5,250);pdf.setFont('helvetica','bold');pdf.setFontSize(8.5);pdf.text(`Total purchases: ${pdfMoney(statement.summary.purchaseTotal)}`,196,y,{align:'right'});pdf.text(`Payments received: ${pdfMoney(statement.summary.paymentsReceived)}`,196,y+6,{align:'right'});pdf.setTextColor(190,24,24);pdf.text(`BALANCE DUE: ${pdfMoney(statement.summary.balance)}`,196,y+12,{align:'right'});const {default:QRCode}=await import('qrcode');const issuedAt=new Date().toISOString();const qr=await QRCode.toDataURL(`ADMABS|CUSTOMER-STATEMENT|${sel._id}|${issuedAt}|${statement.summary.balance}`,{width:320,margin:1,errorCorrectionLevel:'M'});pdf.addImage(qr,'PNG',14,y-3,28,28);pdf.setTextColor(70);pdf.setFont('helvetica','normal');pdf.setFontSize(7);pdf.text('Unique ADMABS document QR',14,y+28);
    const pageTotal=pdf.getNumberOfPages();for(let pageNumber=1;pageNumber<=pageTotal;pageNumber+=1){pdf.setPage(pageNumber);pdf.setDrawColor(20,55,130);pdf.setLineWidth(1.6);pdf.line(12,286,198,286);pdf.setTextColor(45,55,72);pdf.setFont('helvetica','normal');pdf.setFontSize(6.8);pdf.text(`Copyright ${new Date().getFullYear()} ${site.companyName||'ADMABS Investment Company Limited'}. All rights reserved.`,12,291);pdf.text('Computer-generated document | Verify using the unique QR code',pageWidth/2,291,{align:'center'});pdf.text(`Page ${pageNumber} of ${pageTotal}`,198,291,{align:'right'})}
    const safeName=sel.name.replace(/[^a-z0-9]+/gi,'-').replace(/^-|-$/g,'');const filename=`ADMABS-Statement-${safeName}.pdf`;const blob=pdf.output('blob');if(mode==='view'){const previewUrl=URL.createObjectURL(blob);if(previewWindow)previewWindow.location.href=previewUrl;else window.location.href=previewUrl;setTimeout(()=>URL.revokeObjectURL(previewUrl),120000);return}const file=new File([blob],filename,{type:'application/pdf'});const message=`ADMABS customer account statement for ${sel.name}. Account balance due: ${fmt(statement.summary.balance)}.`;
    ...
    if(mode==='whatsapp'){
      if(navigator.share&&navigator.canShare?.({files:[file]})){await navigator.share({title:'ADMABS Customer Statement',text:message,files:[file]});return}
      downloadPdf();const phone=String(sel.phone||'').replace(/\D/g,'');window.open(`https://wa.me/${phone}?text=${encodeURIComponent(`${message}\n\nThe PDF has been downloaded. Please attach ${filename}.`)}`,'_blank');return;
    }
    if(mode==='email'){
      downloadPdf();window.location.href=`mailto:${encodeURIComponent(sel.email||'')}?subject=${encodeURIComponent('ADMABS Customer Account Statement')}&body=${encodeURIComponent(`${message}\n\nPlease attach the downloaded statement: ${filename}`)}`;return;
    }
  };

  return (
    <div className="space-y-5">
      {statementActionsHost&&createPortal(<><button onClick={()=>shareStatement('view')} className="border border-blue-200 bg-white text-blue-800 rounded-lg px-3 py-2 text-xs font-bold">View PDF</button><button onClick={()=>shareStatement('whatsapp')} className="bg-green-600 text-white rounded-lg px-3 py-2 text-xs font-bold"><Send size={13} className="inline mr-1"/>WhatsApp</button><button onClick={()=>shareStatement('email')} className="bg-blue-700 text-white rounded-lg px-3 py-2 text-xs font-bold">Email</button></>,statementActionsHost)}
      {statement&&sel&&statementQr&&<div className="statement-qr-document hidden text-center"><img src={statementQr} alt="Unique statement QR" className="w-24 h-24 mx-auto"/><p className="text-xs">Unique ADMABS document QR</p></div>}
      {statement&&sel&&<div className="statement-document hidden bg-white text-black p-8"><div className="text-center border-b-2 border-blue-900 pb-4 mb-5"><img src={site.logoUrl||'/admabs-logo.png'} alt="ADMABS" className="h-20 max-w-72 object-contain mx-auto"/><h1 className="text-xl font-black text-blue-950">{site.companyName||'ADMABS INVESTMENTS LTD.'}</h1><p className="text-xs">HEAD OFFICE: {site.address||'Ghana'} · {[site.phone,site.email].filter(Boolean).join(' · ')}</p><h2 className="font-black mt-4">CUSTOMER ACCOUNT STATEMENT</h2></div><div className="flex justify-between gap-6 text-sm mb-4"><div className="space-y-1"><p><b>CUSTOMER NAME:</b> {sel.name||'Not provided'}</p><p><b>PHONE NUMBER:</b> {sel.phone||'Not provided'}</p><p><b>CUSTOMER LOCATION:</b> {customerLocation}</p><p className="text-red-700"><b>ACCOUNT BALANCE DUE:</b> {fmt(statement.summary.balance)}</p></div><div className="text-right"><p>Generated: {new Date().toLocaleString()}</p><p>Period: {statementFilters.start||'Beginning'} — {statementFilters.end||'Today'}</p></div></div><table className="w-full text-xs"><thead><tr>{['Date / Reference','Branch / Outlet','Transaction','Customer Debt','Customer Payments','Balance Due'].map(h=><th key={h} className="border p-2 text-left">{h}</th>)}</tr></thead><tbody>{statement.purchases.map(x=><tr key={x._id}><td className="border p-2">{new Date(x.createdAt).toLocaleDateString()}<br/>{x.invoiceNumber}</td><td className="border p-2">{x.branch?.name||'Head Office'}<br/>{x.outlet?.name||''}</td><td className="border p-2">Purchase</td><td className="border p-2">{fmt(x.total)}</td><td className="border p-2">{fmt(x.paidAtSale+x.paidLater)}</td><td className="border p-2">{fmt(x.amountDue)}</td></tr>)}{statement.payments.map(x=><tr key={x._id}><td className="border p-2">{new Date(x.receivedAt).toLocaleDateString()}<br/>{x.receiptNumber}</td><td className="border p-2">{x.branch?.name||'Head Office'}<br/>{x.outlet?.name||''}</td><td className="border p-2">Payment · {x.method}</td><td className="border p-2">—</td><td className="border p-2">{fmt(x.amount)}</td><td className="border p-2">—</td></tr>)}</tbody></table><div className="mt-5 ml-auto max-w-xs border-t-2 pt-3 text-sm"><div className="flex justify-between"><span>Total purchases</span><b>{fmt(statement.summary.purchaseTotal)}</b></div><div className="flex justify-between"><span>Payments received</span><b>{fmt(statement.summary.paymentsReceived)}</b></div><div className="flex justify-between text-red-700 text-base"><span>Balance due</span><b>{fmt(statement.summary.balance)}</b></div></div><p className="text-center text-xs mt-8 border-t pt-3">This computer-generated statement is issued by ADMABS Investments Ltd.</p></div>}
...
function RepView({ user }) {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap justify-between gap-3"><div><h2 className="text-xl font-black text-blue-900">Financial Analytics & Forecast</h2><p className="text-sm text-gray-500">Filter actual sales, margins, branches and products; forecast uses recent weighted growth with a ±15% range</p></div><div className="flex gap-2"><button onClick={exportCsv} disabled={!d} className="px-3 py-2 border border-blue-200 text-blue-900 rounded-xl text-xs font-bold disabled:opacity-40">Download CSV</button><button onClick={printReport} disabled={!d} className="px-3 py-2 bg-blue-900 text-white rounded-xl text-xs font-bold disabled:opacity-40"><Printer size={13} className="inline mr-1"/>View / print report</button></div></div>
...
export default function DesktopApp() {
  ...
  if (!role) return <LoginScreen onLogin={account => { setUser(account);setWelcomeVisible(true);localStorage.setItem('admabs_session_user',JSON.stringify(account)); const allowed = modulesForUser(account); const saved=localStorage.getItem('admabs_active_module');setActive(saved&&allowed.includes(saved)?saved:(allowed[0] || 'dash')); }} />;
  ...
  return (
    <div className="admabs-app flex h-screen bg-gray-50 overflow-hidden" style={{ fontFamily: 'system-ui,-apple-system,sans-serif' }}>
      <GlobalHelpTooltip/>
      {welcomeVisible&&<div className="fixed right-5 top-20 z-[80] w-[min(92vw,430px)] overflow-hidden rounded-2xl border border-blue-300 bg-gradient-to-r from-blue-950 via-blue-900 to-blue-700 p-5 text-white shadow-2xl admabs-welcome"><button onClick={()=>setWelcomeVisible(false)} aria-label="Close welcome message" className="absolute right-3 top-3 text-blue-200 hover:text-white"><X size={15}/></button><p className="text-lg font-black">{greeting}, {user?.name}!</p><p className="mt-1 text-sm font-black text-blue-100">Welcome to the ADMABS Business Platform</p><div className="mt-3 flex flex-wrap gap-x-4 gap-y-1 text-xs font-black"><span>{ghanaNow.toLocaleDateString('en-GH',{timeZone:'Africa/Accra',weekday:'long',day:'numeric',month:'long',year:'numeric'})}</span><span>{ghanaNow.toLocaleTimeString('en-GH',{timeZone:'Africa/Accra'})} · Ghana time</span></div></div>}
