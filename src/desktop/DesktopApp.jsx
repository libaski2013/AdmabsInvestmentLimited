...
function CustView({ user }) {
  ...
  const [statement,setStatement]=useState(null); const [statementFilters,setStatementFilters]=useState({start:'',end:'',branch:'',outlet:''}); const [payment,setPayment]=useState(null); const [paymentReceipt,setPaymentReceipt]=useState(null); const [site,setSite]=useState({}); const [statementActionsHost,setStatementActionsHost]=useState(null); const [statementPrintHost,setStatementPrintHost]=useState(null); const [statementQr,setStatementQr]=useState('');
  ...
  useEffect(()=>{if(!statement||!sel){setStatementPrintHost(null);setStatementQr('');return}setStatementPrintHost(document.querySelector('.statement-document'));import('qrcode').then(({default:QRCode})=>QRCode.toDataURL(`ADMABS|CUSTOMER-STATEMENT|${sel._id}|${Date.now()}|${statement.summary.balance}`,{width:320,margin:1,errorCorrectionLevel:'M'})).then(setStatementQr).catch(()=>{})},[statement,sel]);
  ...
  const shareStatement=async(mode='share')=>{
    ...
    const pdf=new jsPDF({unit:'mm',format:'a4'}), pageWidth=210; let y=16;
    try{const logo=new Image();logo.crossOrigin='anonymous';logo.src=site.logoUrl||'/admabs-logo.png';await new Promise((resolve,reject)=>{logo.onload=resolve;logo.onerror=reject});pdf.addImage(logo,'PNG',65,y,80,24);y+=29}catch{pdf.setFontSize(22);pdf.setTextColor(20,55,130);pdf.text('ADMABS',pageWidth/2,y+8,{align:'center'});y+=18}
    ...
    pdf.setFont('helvetica','normal');pdf.setFontSize(8);pdf.text(`HEAD OFFICE: ${site.address||'Ghana'}  ${[site.phone,site.email].filter(Boolean).join(' | ')}`,pageWidth/2,y,{align:'center',maxWidth:185});y+=10;
    ...
    const headers=['Date / Reference','Branch / Outlet','Transaction','Debit','Credit','Due']; const xs=[14,48,93,124,150,176];pdf.setFillColor(20,55,130);pdf.rect(12,y,186,8,'F');pdf.setTextColor(255);pdf.setFont('helvetica','bold');headers.forEach((h,i)=>pdf.text(h,xs[i],y+5));y+=10;pdf.setTextColor(15,23,42);pdf.setFont('helvetica','normal');
    const rows=[...statement.purchases.map(x=>[`${new Date(x.createdAt).toLocaleDateString()} ${x.invoiceNumber}`,`${x.branch?.name||'Head Office'} / ${x.outlet?.name||''}`,'Purchase',fmt(x.total),fmt(x.paidAtSale+x.paidLater),fmt(x.amountDue)]),...statement.payments.map(x=>[`${new Date(x.receivedAt).toLocaleDateString()} ${x.receiptNumber}`,`${x.branch?.name||'Head Office'} / ${x.outlet?.name||''}`,`Payment - ${x.method}`,'-',fmt(x.amount),'-'])];
    for(const row of rows){if(y>278){pdf.addPage();y=16}row.forEach((value,i)=>pdf.text(String(value),xs[i],y,{maxWidth:i<3?40:25}));pdf.setDrawColor(225);pdf.line(12,y+3,198,y+3);y+=9}
    y=Math.min(y+5,260);pdf.setFont('helvetica','bold');pdf.text(`Total purchases: ${fmt(statement.summary.purchaseTotal)}`,196,y,{align:'right'});pdf.text(`Payments received: ${fmt(statement.summary.paymentsReceived)}`,196,y+6,{align:'right'});pdf.setTextColor(190,24,24);pdf.text(`BALANCE DUE: ${fmt(statement.summary.balance)}`,196,y+12,{align:'right'});const {default:QRCode}=await import('qrcode');const issuedAt=new Date().toISOString();const qr=await QRCode.toDataURL(`ADMABS|CUSTOMER-STATEMENT|${sel._id}|${issuedAt}|${statement.summary.balance}`,{width:320,margin:1,errorCorrectionLevel:'M'});pdf.addImage(qr,'PNG',14,y-3,28,28);pdf.setTextColor(70);pdf.setFont('helvetica','normal');pdf.setFontSize(7);pdf.text('Unique ADMABS document QR',14,y+28);
  ...
  return (
    <div className="space-y-5">
      {statement&&sel&&<div className="flex justify-end"><button onClick={shareStatement} className="flex items-center gap-2 px-4 py-2.5 bg-green-600 text-white rounded-xl text-xs font-black shadow-sm"><Send size={14}/> Share PDF via WhatsApp / Email</button></div>}
      {statementActionsHost&&createPortal(<><button onClick={()=>shareStatement('view')} className="border border-blue-200 text-blue-800 rounded-lg px-3 py-2 text-xs font-bold">View PDF</button><button onClick={()=>shareStatement('share')} className="bg-green-600 text-white rounded-lg px-3 py-2 text-xs font-bold"><Send size={13} className="inline mr-1"/>Share PDF</button></>,statementActionsHost)}
      {statementPrintHost&&statementQr&&createPortal(<div className="mt-5 text-center"><img src={statementQr} alt="Unique statement QR" className="w-24 h-24 mx-auto"/><p className="text-xs">Unique ADMABS document QR</p></div>,statementPrintHost)}
...
function RepView({ user }) {
  ...
  const [filters,setFilters]=useState({start:six.toISOString().slice(0,10),end:now.toISOString().slice(0,10),branch:'',outlet:'',product:'',shift:''}); const [d,setD]=useState(null); const [branches,setBranches]=useState([]); const [outlets,setOutlets]=useState([]); const [err,setErr]=useState('');
